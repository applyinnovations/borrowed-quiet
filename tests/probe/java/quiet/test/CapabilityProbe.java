  ...
  public void runTest(ClientGameTestContext context) {
    try (var world = context.worldBuilder().create()) {
      ...
      world
          .getServer()
          .runCommand(
              "execute at @a run playsound minecraft:block.amethyst_block.chime master @a ~2 ~ ~"
                  + " 0.5 1");
      context.waitTicks(100);
      context.runOnClient(
          client ->
              System.out.println(
                  "CAPABILITY_AUDIO master="
                      + client.options.getSoundSourceVolume(net.minecraft.sounds.SoundSource.MASTER)
                      + " listener="
                      + client.getSoundManager().getListenerTransform()
                      + " player="
                      + client.player.position()));
      world
          .getServer()
          .runCommand(
              "execute at @a run playsound minecraft:entity.cow.ambient master @a ~ ~ ~ 1 1");
