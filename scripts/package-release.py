...
bundle = output / "borrowedquiet-1.0.0.git.bundle"
...
for directory in (root / "evidence/runtime", root / "evidence/reviews"):
    recordings.update(file for file in directory.rglob("*") if file.is_file())
archives = []
for name, members in (("source", files), ("evidence", recordings)):
    archive_path = output / f"borrowedquiet-1.0.0-{name}.zip"
    with zipfile.ZipFile(
        archive_path, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=6
    ) as archive:
        for file in sorted(members):
            relative = (
                pathlib.Path("history.git.bundle") if file == bundle else file.relative_to(root)
            )
            assert not any(
            ...
            info.compress_type = (
                zipfile.ZIP_STORED
                if file.suffix in {".mkv", ".ogg", ".png", ".bundle"}
...
checksums = []
for file in (output / jar.name, bundle, *archives):
